...
Currently all nodes are running on Jetstream at TACC (Texas Advanced Computing Center). Eventually additional Solr nodes and at least one Zookeeper node will be run on the Indiana University Jetstream.

## Zookeeper

[Apache Zookeeper](https://zookeeper.apache.org) is a service for configuring clusters.  In our case Zookeeper stores Solr configuration data and the only configuration a Sorl node needs is the address of the Zookeeper service.
